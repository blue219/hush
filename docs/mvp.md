...
Home, active meditation, completion, and history replay use the same seeded 600-star galaxy, with blue/lavender bloom, fine trails, and sparse star flares. Home runs a decorative calm preview. Home, meditation, and completion share motion state across route changes; completion freezes the final shape. History replay derives a deterministic still frame from the selected recorded second and measured bands. These frames are artistic representations, not exact recordings of live particle positions.

The shared galaxy includes 48 seeded, multi-scale nebula wisps behind the stars and 12 foreground dust patches. Cached unit-gradient brushes are transformed for drawing rather than rebuilt per cloud per frame. Cloud breathing and drift use the existing motion phase, so pause, completion, and missing-signal shape freezes include the nebula. This is layered 2D emission/occlusion, not volumetric simulation; GPU frame timing still requires device measurement.

The galaxy has four narrow spiral arms with dark space between them. Occasional outlying stars soften the edges; smaller, dimmer nebula wisps follow the arms without filling the gaps. Glow and trails are limited to one in six stars.
